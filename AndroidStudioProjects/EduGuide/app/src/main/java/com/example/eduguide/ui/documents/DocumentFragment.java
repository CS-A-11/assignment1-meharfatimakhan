...
public class DocumentFragment extends Fragment {
    ...
    MyDocumentAdapter adapterMyDocAssignment;
    MyDocumentAdapter adapterMyDocQuiz;
    MyDocumentAdapter adapterMyDocPP;
    MyDocumentAdapter adapterMyOtherMaterial;
    ...
    private void initRecyclerView() {
        ...
        for (int i = 0; i < documentList.size();i++){
            if (documentList.get(i).getDocType().equals("Quiz")) {
                ...
                myDocQ.add(docObj);
                adapterMyDocQuiz = new MyDocumentAdapter(getActivity(),myDocQ);
            }
            else if (documentList.get(i).getDocType().equals("Assignment")){
                docObj = documentList.get(i);
                myDocA.add(docObj);
                adapterMyDocAssignment = new MyDocumentAdapter(getActivity(),myDocA);
            }
            else if (documentList.get(i).getDocType().equals("Past Paper")){
                docObj = documentList.get(i);
                myDocP.add(docObj);
                adapterMyDocPP = new MyDocumentAdapter(getActivity(),myDocP);
            }
            else if (documentList.get(i).getDocType().equals("Other Material")){
                docObj = documentList.get(i);
                myDocO.add(docObj);
                adapterMyOtherMaterial = new MyDocumentAdapter(getActivity(),myDocO);
            }
        }
